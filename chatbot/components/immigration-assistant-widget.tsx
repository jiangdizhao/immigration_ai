...
function processingStage(elapsedMs: number, isZh: boolean) {
  const stages = isZh ? PROCESSING_STAGES_ZH : PROCESSING_STAGES_EN;
  return stages.reduce((current, stage) =>
    elapsedMs >= stage.afterMs ? stage : current
  );
}
...
function AssistantProcessingCard({
  ...
  return (
    <div className="inline-flex max-w-[92%] items-start gap-3 rounded-3xl border border-slate-200 bg-white px-4 py-3 text-[15px] text-slate-700 shadow-sm sm:text-base">
...
export function ImmigrationAssistantWidget() {
  ...
  const handleSubmitDraftFacts = async () => {
    ...
    const visibleUserMessage: Extract<WidgetMessage, { role: "user" }> = {
      id: generateUUID(),
      role: "user",
      text: visibleText,
    };

    const backendUserMessage: Extract<WidgetMessage, { role: "user" }> = {
      id: generateUUID(),
      role: "user",
      text: syntheticText,
    };
  ...
  const pendingIsZh =
    latestUserMessage?.role === "user"
      ...
      : false;
  const pendingStage = processingStage(pendingElapsedMs, pendingIsZh);
