...
export default function LawyerReviewPage() {
  ...
  async function loadQueue() {
    ...
    try {
      ...
      const data = await api(`?status=${encodeURIComponent(status)}&limit=80`);
    ...
    <main className="min-h-screen bg-slate-50 px-6 py-8 text-slate-900">
      <div className="mx-auto max-w-7xl space-y-6">
        <header className="rounded-3xl border bg-white p-6 shadow-sm">
          ...
          <h1 className="mt-2 text-3xl font-bold">Answer audit and feedback queue</h1>
              ...
              {queue.map((item) => (
                ...
                >
                  <div className="flex items-center justify-between gap-3">
                    <span className="font-semibold">{item.issue_type || "Unclassified"}</span>
                    <span className="rounded-full bg-slate-100 px-2 py-1 text-xs">
                      {item.review_status || "unreviewed"}
                    </span>
                  </div>
                  <p className="mt-2 text-slate-600">{preview(item.user_message)}</p>
                  <p className="mt-1 text-xs text-slate-400">
                    matter {item.matter_id.slice(0, 8)} · {item.confidence || "no confidence"} · reviews {item.review_count ?? 0}
                  </p>
                </button>
              ))}
              {!queue.length ? <p className="text-sm text-slate-500">No traces loaded.</p> : null}
            </div>
          </section>
