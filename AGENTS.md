...
### Phase 7.3A rule-formation and ReasoningBank invariants

- Phase 7.3A is offline/control-plane only: candidate → allowlisted compiler packet → strict rule proposal → explicit governance decision → immutable structured rule. The compiler has no provider/model/retrieval/evidence dependency and stores no hidden reasoning or raw case conversation.
- Rules are procedural only (`research_strategy`, `evidence_strategy`, `fact_elicitation`, `reasoning_strategy`, `failure_avoidance`). Deterministic quality/case-erasure gates are hard-residue checks, not proof of legal correctness or generalization; transfer validation is deferred to Phase 7.3B.
- Reuse immutable `ReviewArtifact` storage with explicit proposal/decision/rule logical IDs and versions; do not add tables or migrations. Lock every involved parent `AnswerReview` in sorted order. Canonical rule payloads contain source IDs, not evidence, citations, questions, answers, or mutable usage counters.
- Keep `real` and `simulation` namespaces strictly isolated. Real rule writes require the trusted lawyer assertion and live lawyer-reviewed provenance; simulation writes are internal/offline. No namespace conversion or cross-namespace support/conflict exists.
- 7.3A may create only `approved/unvalidated` and `retired` rules. It must not create `shadow` or `active`, auto-merge, auto-retire, retrieve, embed, inject, or read rules from customer/V2/Phase-6 serving paths. Governance actions are explicit (`approve_new`, `merge_support`, `revise_existing`, `mark_conflict`, `reject`).
- 7.3A candidate histories, proposal histories, rule histories, decisions, and evaluation links fail closed on hash/version ambiguity. Experience support is derived from trace-compatible candidate lineage; REAL evaluation support uses active default-regression eligibility. Governance mutations flush inside their savepoint, and API commit/database failures roll back.
- Proposal identity uses normalized semantic compiler-draft fields; decision reason codes participate in terminal identity; bank digests represent normalized logical current rule state rather than timestamps or storage UUIDs. Canonical governance artifacts contain no free-form case narrative; Phase 7.2 review records remain the home for lawyer commentary.

### Phase 7.3B synthetic self-evolution invariants

- Phase 7.3B is an experiment-only control plane. It uses a versioned fictional synthetic regulatory micro-world (`NOT AUSTRALIAN LAW`, `NOT LEGAL AUTHORITY`), never customer requests, Australian legal evidence, real-bank rules, or serving prompts.
- Every family has separate source, held-out positive-transfer, and negative-control cases. Experiment metadata is strictly separated from `SyntheticTaskVisibleInput`/`RetrievalQuery`: task IDs, family/split labels, fixture roles, and `SyntheticTaskOracle` must never enter runner prompts, memory guidance, or retriever input. The Phase-7.3B compiler adapter rejects scored holdout/negative inputs.
- Fixture mode uses deterministic fake providers and may use explicitly tagged forced failures; v1 is infrastructure-only and v2 is the independently authored `live_efficacy_pilot` pack. Live mode requires both `--live` and `PHASE7_3B_LIVE_ENABLED=true`, the live-efficacy pack, explicit compiler/runner models, tool-free provider calls, preflighted cumulative scheduling, actual repeat execution, and a hard call cap. Normal tests make no provider calls.
- Simulation artifacts use only a temporary SQLite store with an explicit non-authoritative URL guard and a simulation-only no-op advisory-lock function. They must never write to configured PostgreSQL and must retain `synthetic_test` provenance plus `simulation_offline` governance.
- Only a strict ReasoningGuidancePacket reaches the memory condition. It contains procedural fields and retrieval metadata only: no benchmark taxonomy, source case, feedback, evidence, citations, source chunks, or legal identifiers. Source-content residue is rejected at proposal governance and checked again before guidance use. A memory rule can never be cited as evidence.
- Retrieval is deterministic lexical scoring with one frozen threshold and `top_k <= 3`, filtered to current approved/normal simulation rules. The primary evaluator is deterministic and centrally scores declared material oracle fields including required claims; provider failures are `NOT_SCORED`, source cases are excluded from transfer metrics, no-retrieval precision is null, and fixture runs can report only `INFRASTRUCTURE_VALIDATED`. Live mechanism support requires source failure, compiler/governed-rule evidence, held-out positive transfer, zero material harm/leakage, complete pairs, computable metrics, and no unresolved infrastructure errors.
- Phase 7.3B modules are not imported by query, Default/V2/Premium, research, evidence, finalizer, Phase 6, or RequestEvidenceRegistry code. No lifecycle promotion, real-bank read, embeddings, production learning telemetry, migrations, or serving integration is permitted.


### Shared project state + Phase 11 UI rebase

- This repository now adopts the repository-first shared project-state workflow documented under `docs/agent-memory/`. Conversations are temporary working memory; Git, code, tests and repository state documents are durable state.
- Phase 11 product/UI authority is `docs/architecture/SERVICE_PLATFORM_UI_REBASE_V1.md`. Its current implementation branch is `phase11-chinese-service-platform-ui-rebase`, created from Phase 10.2 commit `3b3653202f9b067fbed4adfd410edc02cb7215cc`.
- Operational state is split intentionally: `PROJECT_STATE.md` is compressed global state, `CURRENT_MILESTONE.md` tracks the active milestone, `CURRENT_HANDOFF.md` is the immediate continuation record, `DECISIONS.md` is the durable decision ledger, and `docs/agent-memory/tasks/*.md` are bounded work orders.
- Before a meaningful implementation task, read this file, `CURRENT_HANDOFF.md`, the active Task Packet and only the referenced authority documents; then verify branch/HEAD/worktree against Git.
- Coding agents must update `CURRENT_HANDOFF.md` before ending a meaningful task or deliberately starting a fresh session. Their narrative is advisory until checked against actual source, diff and tests.
- The temporary `jiangdizhao/immigration_temporal_ui` repository is a Phase 11 design/journey reference only. Do not copy prototype code/data wholesale or treat mock credentials, addresses, statistics, SLA/privilege wording or testimonials as verified production facts.
- Phase 11 UI work does **not** authorize changes to Fast/Default/Premium legal reasoning, Phase-6/evidence semantics, ReasoningBank governance, political/privacy gating, database schema, billing/email infrastructure or AWS deployment unless an explicit later Task Packet and user approval say otherwise.
