...
def _run(provider: FakeProvider, *, budget: ExecutionBudget | None = None, flat_calls: list[str] | None = None):
    settings = Settings(
...
def test_submit_remains_allowed_after_two_research_rounds():
    ...
    assert result.status == "completed"
    assert result.metrics.tool_round_count == 2
    assert result.metrics.submit_answer_call_count == 1
...
def test_checker_runs_after_substantive_submission_on_revised_arm():
    ...
    assert result.checker_dropped_claim_ids == ["c2"]
    assert result.metrics.fact_check_latency_ms >= 0
    assert provider.seen_tools[1] == ["submit_compact_checker_result"]
