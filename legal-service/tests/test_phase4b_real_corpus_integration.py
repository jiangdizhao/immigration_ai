...
@pytest.mark.parametrize("schedule", ["1", "2", "3", "8"])
def test_schedule_lookup_returns_registered_real_canonical_rows(
    ...
    for match in output.matches:
        ...
        assert db.get(LegalSource, evidence.canonical_source_id) is not None
        assert db.get(SourceChunk, evidence.canonical_chunk_id) is not None


@pytest.mark.parametrize(
    ("document_id", "expected_family"),
    [
...
def test_act_and_regulations_lookup_are_coverage_scoped(
    canonical_read_only_db, document_id: str, expected_family: str
):
    db, _ = canonical_read_only_db
    output, _ = _lookup(
        db,
