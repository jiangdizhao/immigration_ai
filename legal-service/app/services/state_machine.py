...
class StateMachine:
    ...
    def _carry_forward_facts_for_standalone_turn(
        ...
        """
        Carry historical facts and active-frame metadata into the next turn.

        There is no truly standalone legal frame inside an ongoing matter. Earlier
        pruning dropped active_case_frame_id and urgent facts such as
        student_visa_expired_days, causing short fact updates like "I am in
        Australia" to drift into a generic frame. CaseFrameService now validates
        and removes forbidden facts after all history has been carried forward.
        """
        ...
        *,
        ...
        return RiskFlags(
            deadline_sensitive=(
                ...
                or (
                    bool(known_facts.get("notification_date"))
                    and operation_type in {"review_rights", "review_deadline", "student_refusal_next_steps"}
                )
            ),
