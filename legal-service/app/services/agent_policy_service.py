            ...
            "requests": {
                ...
                "items": {
                    ...
                    "properties": {
                        ...
                        "source_type": {"anyOf": [{"type": "string", "maxLength": 100}, {"type": "null"}]},
                        "locator_type": {"anyOf": [{"type": "string", "maxLength": 100}, {"type": "null"}]},
                        "locator": {"anyOf": [{"type": "string", "maxLength": 2000}, {"type": "null"}]},
                        "target_document": {"anyOf": [{"type": "string", "maxLength": 500}, {"type": "null"}]},
                        "node_type": {"anyOf": [{"type": "string", "maxLength": 100}, {"type": "null"}]},
                        "provision_ref": {"anyOf": [{"type": "string", "maxLength": 255}, {"type": "null"}]},
                    ...
                    "required": [
                        "query", "document_id", "source_types", "source_type", "locator_type",
                        "locator", "target_document", "node_type", "provision_ref", "schedule", "provision",
...
- When the matter genuinely falls within Schedule-2 coverage (such as a visa subclass, Schedule-2 criterion, or provision chain), prefer `schedule2_navigation` as the structural orientation before broader evidence where useful. It is not required outside that coverage and need not be the literal first tool call. Its nodes, edges, relation names, local-availability flags, and provenance are navigation hints only—not legal evidence, authority, eligibility, applicability, or a conclusion. Do not cite graph output or mechanically search every graph target.
- Consider one bounded `exact_legal_lookup` batch when research identifies a concrete material legal locator and canonical local source text would close an actual gap. Consider `native web_search` for current, external, open-world, amendment/transitional, case/tribunal, guidance, instrument, or locally unresolved material. Consider `flat_rag_search` only for a specific local semantic/discovery gap; it is not mandatory for every legal question.
- Every research tool call must resolve a specific material information gap. Before using a tool, identify the gap it is intended to close and choose the most direct available tool. Do not batch overlapping research tools merely because they are available. Use multiple tools in one round only for genuinely distinct gaps; after each result, reassess sufficiency and stop when further research is not materially necessary.
- Choose the minimum sufficient tools dynamically. There is no mandatory tool combination or sequence, and no separate research plan. Do not force local retrieval, exact lookup, and web search together when one source or tool closes the material gap. Keep related research actions in the same coherent answer/research stage and respect the available caps.
